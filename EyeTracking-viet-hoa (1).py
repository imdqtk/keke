...
class EyeTracking:
    def __init__(self):
        ...
        # welcome messages
        self.pushText('Chào mừng bạn đến với phần mềm EYE TRACKING của chúng tôi!', 100)
        self.pushText('Nhấn phím <' + KEY_HELP + '> hiển thị danh sách các lệnh.', 150)
    ...
    def showCommandList(self):
            ...
            self.pushText('Nhấn phím <' + KEY_QUIT + '> để thoát EyeTracker.', 150)
            self.pushText('Nhấn phím <' + KEY_CALIBRATE + '> để bắt đầu hiệu chuẩn.', 150)
            self.pushText('Nhấn phím <' + KEY_RESET + '> để đặt lại giá trị hiệu chuẩn.', 150)
            self.pushText('Nhấn phím <' + KEY_MEAN + '> để chuyển đổi bộ lọc trung bình.', 150)
            self.pushText('Nhấn phím <' + KEY_TIME + '> để chuyển đổi bộ lọc thời gian.', 150)
            self.pushText('Nhấn phím <' + KEY_GRID + '> để chuyển đổi bộ lọc lưới.', 150)
            self.pushText('Nhấn phím <' + KEY_SAVE + '> để lưu dữ liệu hiệu chuẩn.', 150)
            self.pushText('Nhấn phím <' + KEY_LOAD + '> để tải dữ liệu hiệu chuẩn.', 150)
            self.pushText('Nhấn phím <' + KEY_USE_HEAD + '> để chuyển đổi sử dụng vị trí đầu.', 150)
    ...
    def updateLabels(self):
        if(self.values is not None):
            self.labelFace.text = 'Mất khuôn mặt: {} khung hình'.format(int(self.lost[0,0]))
            self.labelLeftEye.text = 'Mắt trái bị mất: {} khung hình'.format(int(self.lost[2,0]))
            self.labelRightEye.text = 'Mắt phải bị mất: {} khung hình'.format(int(self.lost[4,0]))
        self.labelThreshold.text = 'Ngưỡng: {}'.format(self.detector.threshold)
        self.labelFilterMean.text = 'Bộ lọc trung bình: {}'.format(self.useFilterMean)
        self.labelFilterTime.text = 'Bộ lọc thời gian: {}'.format(self.useFilterTime)
        self.labelFilterGrid.text = 'Lưới lọc: {}'.format(self.useFilterGrid)
        self.labelFPS.text = 'FPS: {}'.format(self.FPS)
        self.labelHead.text = ' Vị trí đầu: {}'.format(self.useHead)
    # render the current output image
    def renderImage(self):
        # Create empty image
        backgroundColor = np.array(COLOR_BACKGROUND, dtype=np.uint8)
        currentImage = np.zeros((self.height, self.width, 3), dtype=np.uint8)
        

        # Check if the user pressed the 's' key, and change the current image to the default background color

            
        # Use the background image if available, otherwise use the default background color
        if self.backgroundImage is not None:
            currentImage = self.backgroundImage
        else:
            currentImage[:, :] = backgroundColor
    ...
    def handleKeys(self, key):
        if(key == ord(KEY_HELP)):
            ...
            if(self.mode == MODE_NORMAL):
                ...
                self.removeRenderObjects()
                self.pushText('Cố gắng điều chỉnh vị trí và ánh sáng của bạn, cho đến khi xác định vị trí mắt chính xác.', 100)
                self.pushText('Nhấn phím <' + KEY_INC + '> hoặc <' + KEY_DEC + '> để thay đổi kích thước hình ảnh.', 100)
                self.pushText('Nhấn phím <' + KEY_NEXT + '> khi bạn đã hoàn thành.', 100)
            else:
                self.calibrationData = self.calibrationDataBackup.copy()
                self.mode = MODE_NORMAL
                self.removeRenderObjects()
                self.pushText('Việc hiệu chuẩn đã bị hủy bỏ!', 100)
        elif(key == ord(KEY_RESET)):
            self.mode = MODE_NORMAL
            self.calibrationData = []
            self.calibrated = False
            self.removeRenderObjects()
            self.pushText('Đã xóa tất cả dữ liệu hiệu chuẩn.', 100)
        elif(key == ord(KEY_MEAN)):
            newImage = cv2.imread("face.png")
            ...
            if(self.mode == MODE_CAMERA_SIZE):
                ...
                self.removeRenderObjects()
                self.pushText('Đưa con ngươi vào vùng phát hiện chính xác nhất.', 100)
                self.pushText('Bấm phím <' + KEY_INC + '> hoặc <' + KEY_DEC + '> để điều chỉnh giá trị ngưỡng.',100)
                self.pushText('Bấm phím N khi bạn đã hoàn thành.', 100)
            elif(self.mode == MODE_THRESHOLD):
                self.mode = MODE_CALIBRATE_CURSOR
                self.cursorLocation = 0
                self.removeRenderObjects()
                self.pushText('Bạn hãy hìn theo vòng tròn màu cam .', 100)
                self.pushText('Bấm phím <' + KEY_NEXT + '> khi bạn nhìn vào vòng tròn.', 100)
            elif(self.mode == MODE_CALIBRATE_CURSOR):
                ...
                if(self.cursorLocation == len(self.locations)):
                    ...
                    self.removeRenderObjects()
                    self.pushText('Hiệu chuẩn hoàn tất!', 100)
            ...
            if(self.mode == MODE_NORMAL):
                ...
                if(self.calibrated):
                    with open('calibrationData.dat', 'wb') as fp:
                        ...
                        self.pushText('Đã lưu dữ liệu hiệu chuẩn.', 100)
                else:
                    self.pushText('Lỗi! Chưa hiệu chuẩn thành công.', 100)
            ...
            if(self.mode == MODE_NORMAL):
                ...
                if(os.path.exists('calibrationData.dat')):
                    with open('calibrationData.dat', 'rb') as fp:
                        ...
                        self.computeCalibration()
                        self.calibrated = True
                        self.pushText('Đã tải dữ liệu hiệu chuẩn.', 100)
                else:
                    self.pushText('Lỗi! Không có dữ liệu hiệu chuẩn.', 100)
    ...
    def run(self):
        ...
        while(True):            
            ...
            if(returnValue == 0): # no image could be read
                self.pushText('Không tìm thấy webcam!', 50)
                self.mode = MODE_NORMAL
                self.calibrationData = []
                self.calibrated =  False
